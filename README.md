...
+ [ШАГ 5, ВНЕДРЕНИЕ БЕЗ КОНФЛИКТОВ (ветки копия в ветку master)](#Step5);      🛰️
...
##### <a name="Step5"></a> ШШАГ 4, ЗАМЕНА РАБОЧИХ ФАЙЛОВ НА ИЗМЕНЕННЫЕ 🏁 [(начало)](#pageup);

# ШШАГ 4, ЗАМЕНА РАБОЧИХ ФАЙЛОВ НА ИЗМЕНЕННЫЕ:

1. Через конфигуратор выполнил сравнить и объединить (для устранения конфликтов)
    
        Если Конфликт Тогда
            Выполняем СЛИЯНИЕ веток (завершаем ветки):
            ITTF_XX + copy_xxxxxxxx_x"
            master  + copy_xxxxxxxx_x"
            РЕШАЕМ конфликт через конфигуратор (оставляем нужное и удаляем не нужное)
            Удаляем ветку задач ITTF_xx
...
##### <a name="Step5"></a>	ШАГ 5, ВНЕДРЕНИЕ БЕЗ КОНФЛИКТОВ _(ветки копия в ветку master)_ [(начало)](#pageup);

# ШАГ 5, ВНЕДРЕНИЕ БЕЗ КОНФЛИКТОВ _(ветки копия в ветку master)_ 🛰️
...
##### <a name="Step5_1"></a>	**СЛИЯНИЕ ВЕТОК** master и copy_xxxxxxxx_xx [(начало)](#pageup);
